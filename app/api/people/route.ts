...
async function finish(operation: "select" | "insert" | "update" | "delete", started: number, response: Response, recordId?: number) {
  ...
  await writeQueryLog({ operation, table: "people", status: response.ok ? "success" : "error", duration_ms: Date.now() - started, record_id: recordId, row_count: rowCount, error });
  return NextResponse.json(response.ok ? body : { error }, { status: response.status });
}

export async function GET() {
  const started = Date.now();
  await writeQueryLog({ operation: "select", table: "people", status: "started" });
  const response = await callSupabase("people?select=*&order=created_at.desc", { method: "GET" });
  if (!response) return configurationError();
  return finish("select", started, response);
}

export async function POST(request: NextRequest) {
  const started = Date.now();
  await writeQueryLog({ operation: "insert", table: "people", status: "started" });
  ...
  if (!response) return configurationError();
  return finish("insert", started, response);
}

export async function PATCH(request: NextRequest) {
  const started = Date.now();
  const id = Number(request.nextUrl.searchParams.get("id"));
  await writeQueryLog({ operation: "update", table: "people", status: "started", record_id: id });
  ...
  if (!response) return configurationError();
  return finish("update", started, response, id);
}

export async function DELETE(request: NextRequest) {
  const started = Date.now();
  const id = Number(request.nextUrl.searchParams.get("id"));
  await writeQueryLog({ operation: "delete", table: "people", status: "started", record_id: id });
  if (!Number.isFinite(id)) return NextResponse.json({ error: "올바른 id가 필요합니다." }, { status: 400 });
  const response = await callSupabase(`people?id=eq.${id}`, { method: "DELETE", headers: { Prefer: "return=representation" } });
  if (!response) return configurationError();
  return finish("delete", started, response, id);
}
