...
export default function Home(){
 ...
 async function load(){if(!supabase){setLoading(false);return}setLoading(true);const {data,error}=await supabase.from("people").select("*").order("created_at",{ascending:false});if(error)setMessage(`목록을 불러오지 못했습니다: ${error.message}`);else setPeople((data??[]) as Person[]);setLoading(false)}
 useEffect(()=>{void load()},[]);
 ...
 async function save(e:FormEvent){e.preventDefault();if(!supabase)return;setSaving(true);setMessage("");const result=editing===null?await supabase.from("people").insert(form):await supabase.from("people").update(form).eq("id",editing);setSaving(false);if(result.error){setMessage(`저장하지 못했습니다: ${result.error.message}`);return}close();await load()}
 async function remove(p:Person){if(!supabase||!confirm(`${p.name} 님을 삭제하시겠습니까?`))return;const {error}=await supabase.from("people").delete().eq("id",p.id);if(error)setMessage(`삭제하지 못했습니다: ${error.message}`);else await load()}
 return <main className="shell">
  ...
  <section className="content"><header><div><h1>인명 관리</h1><p>회사 구성원 정보를 한곳에서 관리하세요.</p></div><button className="primary" onClick={create} disabled={!isSupabaseConfigured}>＋ 신규 등록</button></header>
   {!isSupabaseConfigured&&<div className="notice"><strong>Supabase 연결 정보가 필요합니다.</strong><span><code>.env.example</code>을 복사해 <code>.env.local</code>을 만들고 실제 값을 입력하세요.</span></div>}{message&&<div className="notice error">{message}</div>}
   ...
   <div className="panel"><div className="toolbar"><div><h2>인명 목록</h2><span>총 {shown.length}명</span></div><label className="search">⌕<input aria-label="인명 검색" placeholder="이름, 회사, 부서, 직책 검색" value={query} onChange={e=>setQuery(e.target.value)}/></label></div><div className="table"><table><thead><tr><th>이름</th><th>회사</th><th>부서</th><th>직책</th><th>등록일</th><th/></tr></thead><tbody>
    ...
    {loading&&<tr><td colSpan={6} className="empty">목록을 불러오는 중입니다...</td></tr>}{!loading&&!shown.length&&<tr><td colSpan={6} className="empty">{isSupabaseConfigured?"등록된 인명이 없습니다.":"Supabase를 연결하면 데이터가 표시됩니다."}</td></tr>}
   </tbody></table></div></div>
  </section>
